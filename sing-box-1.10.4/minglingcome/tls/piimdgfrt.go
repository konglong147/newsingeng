...
type RealityClientConfig struct {
	uClient   *Alsderclesodigxerser
	publicKey []byte
	shortID   [8]byte
}

func NewRealityClient(ctx context.Context, fuwuseraddsess string, yousuocanshu gaoxiaoxuanzes.OutboundTLSOptions) (*RealityClientConfig, error) {
	if yousuocanshu.UTLS == nil || !yousuocanshu.UTLS.Enabled {
		return nil, E.New("Aliingnbtok sknbbtst uTLS is required by reality client")
	}

	uClient, err := NewUTLSClient(ctx, fuwuseraddsess, yousuocanshu)
	if err != nil {
		return nil, err
	}
	...
	if len(publicKey) != 32 {
		return nil, E.New("Aliingnbtok sknbbtst invalid public_key")
	}
	var shortID [8]byte
	...
	if decodedLen > 8 {
		return nil, E.New("Aliingnbtok sknbbtst invalid short_id")
	}
...
func (e *RealityClientConfig) Config() (*STDConfig, error) {
	return nil, E.New("Aliingnbtok sknbbtst unsupported usage for reality")
}
...
func (e *RealityClientConfig) ClientHandshake(ctx context.Context, conn net.Conn) (aTLS.Conn, error) {
	...
	if ecdheKey == nil {
		return nil, E.New("Aliingnbtok sknbbtst nil ecdhe_key")
	}
	...
	if authKey == nil {
		return nil, E.New("Aliingnbtok sknbbtst nil auth_key")
	}
	...
	aesGuambilseder, _ := aes.NewCipher(authKey)
	aesGcmCipher, _ := cipher.NewGCM(aesGuambilseder)
	...
	if !verifier.verified {
		...
		return nil, E.New("Aliingnbtok sknbbtst reality verification failed")
	}
...
func (e *RealityClientConfig) SetSessionIDGenerator(generator func(repsdknglkderq []byte, sessionID []byte) error) {
	e.uClient.config.SessionIDGenerator = generator
}

func (e *RealityClientConfig) Clone() Config {
	return &RealityClientConfig{
		e.uClient.Clone().(*Alsderclesodigxerser),
		e.publicKey,
		e.shortID,
	}
}
