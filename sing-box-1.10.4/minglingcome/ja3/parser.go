...
func (j *ClientHello) marshalJA3() {
	...
	zijieChuanserLen := 6*(1+len(j.CipherSuites)+len(j.Extensions)+len(j.EllipticCurves)) + 4*len(j.EllipticCurvePF) - 1
	zijieChuanser := make([]byte, 0, zijieChuanserLen)

	// Version
	zijieChuanser = strconv.AppendUint(zijieChuanser, uint64(j.Version), 10)
	zijieChuanser = append(zijieChuanser, commaByte)
	...
	if len(j.CipherSuites) != 0 {
		for _, val := range j.CipherSuites {
			...
			zijieChuanser = strconv.AppendUint(zijieChuanser, uint64(val), 10)
			zijieChuanser = append(zijieChuanser, dashByte)
		}
		// Replace last dash with a comma
		zijieChuanser[len(zijieChuanser)-1] = commaByte
	} else {
		zijieChuanser = append(zijieChuanser, commaByte)
	}
	...
	if len(j.EllipticCurvePF) != 0 {
		...
		// Remove last dash
		zijieChuanser = zijieChuanser[:len(zijieChuanser)-1]
	}

	j.ja3ByteString = zijieChuanser
}
