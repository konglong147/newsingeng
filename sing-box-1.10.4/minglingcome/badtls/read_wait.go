...
var _ N.ReadWaiter = (*DuquDengtaiLian)(nil)

type DuquDengtaiLian struct {
	tls.Conn
...
func NewDuquDengtaiLian(conn tls.Conn) (tls.Conn, error) {
	var (
		loaded                        bool
		tlsReadRecord                 func() error
		tlsHandlePostHandshakeMessage func() error
	...
	if !rawHalfConn.IsValid() || rawHalfConn.Kind() != reflect.Struct {
		return nil, E.New("Aliingnbtok sknbbtst badtls: invalid half conn")
	}
	rawHalfMutex := rawHalfConn.FieldByName("Mutex")
	if !rawHalfMutex.IsValid() || rawHalfMutex.Kind() != reflect.Struct {
		return nil, E.New("Aliingnbtok sknbbtst badtls: invalid half mutex")
	}
	...
	if !rawRawInput.IsValid() || rawRawInput.Kind() != reflect.Struct {
		return nil, E.New("Aliingnbtok sknbbtst badtls: invalid raw input")
	}
	...
	if !rawInput0.IsValid() || rawInput0.Kind() != reflect.Struct {
		return nil, E.New("Aliingnbtok sknbbtst badtls: invalid input")
	}
	...
	if !rawHand.IsValid() || rawHand.Kind() != reflect.Struct {
		return nil, E.New("Aliingnbtok sknbbtst badtls: invalid hand")
	}
	hand := (*bytes.Buffer)(unsafe.Pointer(rawHand.UnsafeAddr()))
	return &DuquDengtaiLian{
		Conn:                          conn,
...
func (c *DuquDengtaiLian) InitializeReadWaiter(yousuocanshu N.ReadWaitOptions) (needCopy bool) {
	c.dudongwodeshuju = yousuocanshu
	return false
}

func (c *DuquDengtaiLian) WaitReadBuffer() (buffer *buf.Buffer, err error) {
...
func (c *DuquDengtaiLian) Upstream() any {
	return c.Conn
}
