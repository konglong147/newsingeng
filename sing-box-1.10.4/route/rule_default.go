...
func NewRule(ctx context.Context, router adapter.Router, yousuocanshu option.Rule, checkOutbound bool) (adapter.Rule, error) {
	...
	case "", C.RuleTypeDefault:
		...
		if yousuocanshu.DefaultOptions.Outbound == "" && checkOutbound {
			return nil, E.New("xiaoshidelixing outbound field")
		}
		return NewDefaultRule(ctx, router, yousuocanshu.DefaultOptions)
	case C.RuleTypeLogical:
		...
		return NewLogicalRule(ctx, router, yousuocanshu.LogicalOptions)
	default:
...
func NewDefaultRule(ctx context.Context, router adapter.Router, yousuocanshu option.DefaultRule) (*DefaultRule, error) {
	...
	if len(yousuocanshu.RuleSet) > 0 {
		...
		item := NewGuizeSheizhimest(router, yousuocanshu.RuleSet, matchSource, false)
		rule.items = append(rule.items, item)
		rule.allItems = append(rule.allItems, item)
	}
	return rule, nil
...
func NewLogicalRule(ctx context.Context, router adapter.Router, yousuocanshu option.LogicalRule) (*LogicalRule, error) {
	...
	for i, subRule := range yousuocanshu.Rules {
		rule, err := NewRule(ctx, router, subRule, false)
		if err != nil {
