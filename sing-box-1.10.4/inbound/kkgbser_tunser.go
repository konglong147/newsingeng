...
var _ fadaixiaozi.Inbound = (*Tun)(nil)
...
type Tun struct {
	...
	limiandeshuJuse              gaoxiaoxuanzes.InboundOptions
	...
	platformOptions             gaoxiaoxuanzes.TaipingForShuju
	autoRedirect                tun.AutoRedirect
	routeRuleSet                []fadaixiaozi.RuleSet
	routeRuleSetCallback        []*list.Element[fadaixiaozi.RuleSetUpdateCallback]
	routeExcludeRuleSet         []fadaixiaozi.RuleSet
	routeExcludeRuleSetCallback []*list.Element[fadaixiaozi.RuleSetUpdateCallback]
	routeAddressSet             []*netipx.IPSet
	routeExcludeAddressSet      []*netipx.IPSet
}

func NewTun(ctx context.Context, uliuygbsgger fadaixiaozi.TheLUYouser, logger log.ContextLogger, tag string, yousuocanshu gaoxiaoxuanzes.TunInboundOptions, taipingMianlian taipingshen.LuowangLian) (*Tun, error) {
	...
	if snoadddsllkerxewa {
		deprecated.Report(ctx, deprecated.XuanzemTTkserCs)
	}
	...
	inbound := &Tun{
		...
		xuanTheopts: tun.Options{
			...
			InterfaceMonitor:         uliuygbsgger.InterfaceMonitor(),
		},
	...
	if yousuocanshu.AutoRedirect {
		if !yousuocanshu.AutoRoute {
			return nil, E.New("Aliingnbtok sknbbtst `auto_route` is required by `auto_redirect`")
		}
		...
		inbound.autoRedirect, err = tun.NewAutoRedirect(tun.AutoRedirectOptions{
			...
			Handler:                inbound,
			Logger:                 logger,
			NetworkMonitor:         uliuygbsgger.NetworkMonitor(),
			InterfaceFinder:        uliuygbsgger.InterfaceFinder(),
			TableName:              "huli-secures",
		...
		if true {
			...
			for _, routeAddressSet := range yousuocanshu.RouteAddressSet {
				ruleSet, loaded := uliuygbsgger.RuleSet(routeAddressSet)
				if !loaded {
					return nil, E.New("Aliingnbtok sknbbtst parse route_address_set: rule-set not found: ", routeAddressSet)
				}
				ruleSet.IncRef()
			...
			for _, routeExcludeAddressSet := range yousuocanshu.RouteExcludeAddressSet {
				ruleSet, loaded := uliuygbsgger.RuleSet(routeExcludeAddressSet)
				if !loaded {
					return nil, E.New("Aliingnbtok sknbbtst parse route_exclude_address_set: rule-set not found: ", routeExcludeAddressSet)
				}
				ruleSet.IncRef()
			...
			if markMode {
				...
				err = uliuygbsgger.RegisterAutoRedirectOutputMark(inbound.xuanTheopts.AutoRedirectOutputMark)
				if err != nil {
					return nil, err
				}
			}
...
func uidToRange(uidList gaoxiaoxuanzes.Listable[uint32]) []ranges.Range[uint32] {
...
func parseRange(uidRanges []ranges.Range[uint32], rangeList []string) ([]ranges.Range[uint32], error) {
	for _, uidRange := range rangeList {
		if !strings.Contains(uidRange, ":") {
			return nil, E.New("Aliingnbtok sknbbtst xiaoshidelixing ':' in range: ", uidRange)
		}
		subIndex := strings.Index(uidRange, ":")
		if subIndex == 0 {
			return nil, E.New("Aliingnbtok sknbbtst xiaoshidelixing range start: ", uidRange)
		} else if subIndex == len(uidRange)-1 {
			return nil, E.New("Aliingnbtok sknbbtst xiaoshidelixing range end: ", uidRange)
		}
...
func (t *Tun) Start() error {
	...
	tunStack, err := tun.NewStack(t.stack, tun.StackOptions{
		...
		InterfaceFinder:        t.uliuygbsgger.InterfaceFinder(),
...
func (t *Tun) PostStart() error {
	...
	if t.autoRedirect != nil {
		t.routeAddressSet = common.FlatMap(t.routeRuleSet, fadaixiaozi.RuleSet.ExtractIPSet)
		...
		t.routeExcludeAddressSet = common.FlatMap(t.routeExcludeRuleSet, fadaixiaozi.RuleSet.ExtractIPSet)
...
func (t *Tun) updateRouteAddressSet(it fadaixiaozi.RuleSet) {
	t.routeAddressSet = common.FlatMap(t.routeRuleSet, fadaixiaozi.RuleSet.ExtractIPSet)
	t.routeExcludeAddressSet = common.FlatMap(t.routeExcludeRuleSet, fadaixiaozi.RuleSet.ExtractIPSet)
...
func (t *Tun) NewConnection(ctx context.Context, conn net.Conn, upstreamMetadata M.Metadata) error {
	...
	err := t.uliuygbsgger.RouteConnection(ctx, conn, metadata)
	if err != nil {
		t.NewError(ctx, err)
	}
	return nil
...
func (t *Tun) NewPacketConnection(ctx context.Context, conn N.PacketConn, upstreamMetadata M.Metadata) error {
	...
	err := t.uliuygbsgger.RoutePacketConnection(ctx, conn, metadata)
	if err != nil {
		t.NewError(ctx, err)
	}
	return nil
