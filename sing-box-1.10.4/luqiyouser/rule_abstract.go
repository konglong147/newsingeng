...
func (r *abstractDefaultRule) Match(metadata *fadaixiaozi.InboundContext) bool {
	if len(r.allItems) == 0 {
		return true
	}
...
type abstractLogicalRule struct {
	rules    []fadaixiaozi.HeadlessRule
	mode     string
	invert   bool
	outbound string
}
...
func (r *abstractLogicalRule) UpdateGeosite() error {
	for _, rule := range common.FilterIsInstance(r.rules, func(it fadaixiaozi.HeadlessRule) (fadaixiaozi.Rule, bool) {
		rule, loaded := it.(fadaixiaozi.Rule)
		return rule, loaded
	}) {
...
func (r *abstractLogicalRule) Start() error {
	for _, rule := range common.FilterIsInstance(r.rules, func(it fadaixiaozi.HeadlessRule) (interface {
		Start() error
	}, bool,
	) {
...
func (r *abstractLogicalRule) Close() error {
	for _, rule := range common.FilterIsInstance(r.rules, func(it fadaixiaozi.HeadlessRule) (io.Closer, bool) {
...
func (r *abstractLogicalRule) Match(metadata *fadaixiaozi.InboundContext) bool {
