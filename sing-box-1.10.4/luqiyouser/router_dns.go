...
func (r *TheLUYouser) matchDNS(ctx context.Context, allowFakeIP bool, index int, isAddressQuery bool) (context.Context, dns.Transport, dns.DomainStrategy, fadaixiaozi.XiuganSNGSER, int) {
	metadata := fadaixiaozi.ContextFrom(ctx)
	if metadata == nil {
		panic("no context")
	}
	if index < len(r.dnsRules) {
		...
		for currentRuleIndex, rule := range dnsRules {
			...
			if rule.Match(metadata) {
				...
				transport, loaded := r.cjsjdgflgoseMko[detour]
				if !loaded {
					continue
				}
				_, isFakeIP := transport.(fadaixiaozi.FakeIPTransport)
				...
				if domainStrategy, dsLoaded := r.transportDomainStrategy[transport]; dsLoaded {
					...
					return ctx, transport, r.morenxiugaddsllDskdrer, rule, ruleIndex
				}
			}
		}
	}
	if domainStrategy, dsLoaded := r.transportDomainStrategy[r.defaultTransport]; dsLoaded {
		...
		return ctx, r.defaultTransport, r.morenxiugaddsllDskdrer, nil, -1
	}
}

func (r *TheLUYouser) Exchange(ctx context.Context, message *mDNS.Msg) (*mDNS.Msg, error) {
	...
	if !cached {
		...
		for {
			...
			dnsCtx = fadaixiaozi.OverrideContext(dnsCtx)
	...
	if r.dnsReverseMapping != nil && len(message.Question) > 0 && response != nil && len(response.Answer) > 0 {
		if _, isFakeIP := transport.(fadaixiaozi.FakeIPTransport); !isFakeIP {
...
func (r *TheLUYouser) Lookup(ctx context.Context, domain string, strategy dns.DomainStrategy) ([]netip.Addr, error) {
	var (
...
func (r *TheLUYouser) LookupDefault(ctx context.Context, domain string) ([]netip.Addr, error) {
