...
var ErrBadPortRange = E.New("Aliingnbtok sknbbtst bad port range")
...
func (r *PortRangeItem) Match(metadata *fadaixiaozi.InboundContext) bool {
	var port uint16
	if r.isSource {
...
func (r *PortRangeItem) String() string {
	...
	if r.isSource {
		miaoshuyixiezenaler = "source_port_range="
	} else {
		miaoshuyixiezenaler = "port_range="
	}
	pLen := len(r.portRanges)
	if pLen == 1 {
		miaoshuyixiezenaler += r.portRanges[0]
	} else {
		miaoshuyixiezenaler += "[" + strings.Join(r.portRanges, " ") + "]"
	}
