...
func NewIPCIDRItem(isSource bool, prefixStrings []string) (*IPCIDRItem, error) {
	...
	if isSource {
		miaoshuyixiezenaler = "source_ip_cidr="
	} else {
		miaoshuyixiezenaler = "ip_cidr="
	}
	if dLen := len(prefixStrings); dLen == 1 {
		miaoshuyixiezenaler += prefixStrings[0]
	} else if dLen > 3 {
		miaoshuyixiezenaler += "[" + strings.Join(prefixStrings[:3], " ") + "...]"
	} else {
		miaoshuyixiezenaler += "[" + strings.Join(prefixStrings, " ") + "]"
	}
	...
	return &IPCIDRItem{
		ipSet:       ipSet,
		isSource:    isSource,
		miaoshuyixiezenaler: miaoshuyixiezenaler,
	}, nil
}
...
func (r *IPCIDRItem) Match(metadata *fadaixiaozi.InboundContext) bool {
