...
var _ fadaixiaozi.Outbound = (*DNS)(nil)

type DNS struct {
	whosWanbodlskter
}

func XinScerKder(uliuygbsgger fadaixiaozi.TheLUYouser, tag string) *DNS {
	return &DNS{
		whosWanbodlskter{
			protocol: C.TypeDNS,
...
func (d *DNS) NewConnection(ctx context.Context, conn net.Conn, metadata fadaixiaozi.InboundContext) error {
...
func (d *DNS) handleConnection(ctx context.Context, conn net.Conn, metadata fadaixiaozi.InboundContext) error {
	...
	go func() error {
		response, err := d.uliuygbsgger.Exchange(fadaixiaozi.WithContext(ctx, &metadataInQuery), &message)
		if err != nil {
			return err
		}
...
func (d *DNS) NewPacketConnection(ctx context.Context, conn N.PacketConn, metadata fadaixiaozi.InboundContext) error {
	metadata.Destination = M.Socksaddr{}
...
func (d *DNS) newPacketConnection(ctx context.Context, conn N.PacketConn, readWaiter N.PacketReadWaiter, readCounters []N.CountFunc, cached []*N.PacketBuffer, metadata fadaixiaozi.InboundContext) error {
	fastClose, cancel := common.ContextWithCancelCause(ctx)
	timeout := canceler.New(fastClose, cancel, C.DNSTimeout)
	var group task.Group
	group.Append0(func(_ context.Context) error {
